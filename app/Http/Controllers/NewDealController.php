    ...
    public function newDeal()
    {
        if (!isset(auth()->user()->tableUsers->table_id)) {
            if (Table_card::where('table_id', auth()->user()->tableUsers->table_id)->value('open') >= 4) {
                ...
                //if user's money are not enough for bet, delete this user from game
                foreach ($players as $player) {
                    if (Table_user::where('user_id', $player)->value('money') < $bet) {
                        Table_user::where('user_id', $player)->delete();
                        User_card::where('user_id', $player)->delete();
                    }
                }
                ...
                $players = Table_user::where('table_id', $table_id)->pluck("user_id"); //all game players after deletion players without money

                $allAvailablePlaces = AllAvailablePlaces::places();


                for ($i = 0; $i < count($allAvailablePlaces); $i++) {
                    if ($currentDealer >= count($allAvailablePlaces)) {
                        $dealer = $allAvailablePlaces[0];
                    } else {
                        if ($allAvailablePlaces[$i] > $currentDealer and !isset($dealer)) {
                            $dealer = $allAvailablePlaces[$i];
                        }
                    }
                }
                ...
                foreach ($players as $player) {
                    ...
                    if ($user_place == $smallBlind) {
                        ...
                        Table_user::where('user_id', $player)->update(['bet' => $bet / 2]);
                    } else if ($user_place == $bigBlind) {
                        Table_user::where('user_id', $player)->decrement('money', $bet);
                        Table_user::where('user_id', $player)->update(['bet' => $bet]);
